...
-type rootset() :: [pid() | atom()] |   %% list of pids/registered names
    processes |
    existing_processes |
    new_processes.
...
-spec enable_trace(pid() | processes | new_processes| existing_processes |
        {children | all_children, pid() | atom()}) -> non_neg_integer();
    ([pid() | atom()]) -> non_neg_integer() | {non_neg_integer(), [pid() | atom()]}.
enable_trace(Rootset) ->
    enable_trace(Rootset, #{set_on_spawn => true}).

-spec enable_trace(pid() | processes | new_processes | existing_processes |
        {children | all_children, pid() | atom()}, trace_options()) -> non_neg_integer();
    ([pid() | atom()], trace_options()) -> non_neg_integer() | {non_neg_integer(), [pid() | atom()]}.
...
-spec disable_trace(pid() | processes | new_processes| existing_processes |
    {children | all_children, pid() | atom()}) -> non_neg_integer();
    ([pid() | atom()]) -> non_neg_integer() | {non_neg_integer(), [pid() | atom()]}.
...
-spec inspect([trace_info()], process, sort_by()) -> #{pid() => profile_result()};
    ([trace_info()], total, sort_by()) -> profile_result().
...
collect_trace(Mod, FunList, Acc) ->
    TracedList = [{Mod, Fun, Arity, Tr} || {Fun, Arity} <- FunList,
        (Tr = combine_trace(erlang:trace_info({Mod, Fun, Arity}, call_memory))) =/= skip],
    TracedList ++ Acc.

combine_trace({call_memory, []}) ->
    skip;
combine_trace({call_memory, Mem}) ->
    case [{Pid, Calls, Mega * 1000000 + Words} || {Pid, Calls, Mega, Words} <- Mem, (Mega + Words) > 0] of
        [] ->
            skip;
        NonZero ->
            NonZero
