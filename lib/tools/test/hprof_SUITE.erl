...
all() ->
    [ad_hoc, sort, rootset, set_on_spawn, live_trace, patterns, processes, server, hierarchy].
...
set_on_spawn(Config) when is_list(Config) ->
    ...
    ?assertMatch({?MODULE, {_, 0}, 1, 14, 14, _}, lists:keyfind(?MODULE, 1, TotalProfile)),
    %% MFA takes 1 more word. This test should be improved to depend less on the internal
    %%  implementation.
