...
-export([
    dist/0, dist/1,
    report_reason/0, report_reason/1,
...
groups() ->
    [
        {dist, [parallel], [dist, report_reason, dist_io_redirect, dist_up_down]},
...
report_reason() ->
    [{doc, "Tests reason reporting"}].

report_reason(Config) when is_list(Config) ->
    Name = peer:random_name(?FUNCTION_NAME),
    {ok, Node} = peer:start_link(#{name => Name, report_reason => true}),
...
multi_node(Config) when is_list(Config) ->
    Nodes = [
        peer:start_link(#{name => peer:random_name(?FUNCTION_NAME),
            wait_boot => {self(), tag}, connection => standard_io})
        || _ <- lists:seq(1, 4)],
    [receive {tag, {started, Node, _}} -> ok end || {ok, Node} <- Nodes],
