...
-type start_options() :: #{
    ...
    register => boolean(),              %% register process locally, true by default unless peer node has
                                        %%  no name. Affects start/start_link return value.
    rsh => rsh(),                       %% remote shell to use for starting a node on a different host
    report_reason => boolean(),         %% false (default): when peer terminates, peer control process
                                        %% stops normally regardless of the reason.
                                        %% true: when peer terminates, origin process
...
init([Name, Notify, Options]) ->
    ...
    if ListenSocket =:= undefined ->
            {ok, #peer_state{node = Name, options = Options, connection = Conn, notify = Notify}};
        true ->
            prim_inet:async_accept(ListenSocket, ?ACCEPT_TIMEOUT),
            {ok, #peer_state{node = Name, options = Options, listen_socket = ListenSocket, notify = Notify}}
    end.
...
handle_info({peer_started, Node, Pid}, State) ->
    receive
        {'EXIT', Pid, normal} ->
            ...
            {noreply, boot_complete(Node, State)}
    end;
...
%% start the peer, maybe wait for peer to boot
start_it(Options, Link) ->
    ...
    Notify =
        case WaitBoot of
            infinity ->
                {self(), make_ref()};
            WaitBoot when is_integer(WaitBoot) ->
                {self(), make_ref()};
            {ReplyTo, Tag} when is_pid(ReplyTo) ->
                {ReplyTo, Tag};
            false ->
                false
        end,
    case start_register(Options, {Node, Rego}, Notify, Link) of
        {ok, Pid} when WaitBoot =:= infinity ->
            {_, Ref} = Notify,
            Mref = erlang:monitor(process, Pid),
            receive
                {Ref, {started, NodeName, Pid}} ->
                    erlang:demonitor(Mref, [flush]),
                    start_success(NodeName, Pid, Rego);
                {'DOWN', Mref, _, _, Reason} ->
                    exit(Reason)
            end;
        {ok, Pid} when is_integer(WaitBoot) ->
            {_, Ref} = Notify,
            Mref = erlang:monitor(process, Pid, [{alias,demonitor}]),
            receive
                {Ref, {started, NodeName, Pid}} ->
                    erlang:demonitor(Mref, [flush]),
                    start_success(NodeName, Pid, Rego);
        ...
        {ok, Pid} ->
            start_success(Node, Pid, Rego);
        Error ->
            Error
    end.

%% based on whether the peer process is registered,
%%  return the registered name (which is the same as peer node,
%%  for simplicity), or a peer process pid
start_success(_Node, Pid, false) ->
    {ok, Pid};
start_success(Node, _Pid, _) ->
    {ok, Node}.

%% Execute gen_server:start or start_link, 3 or 4, depending on what
%%  was requested (registered or not, linked or not)
start_register(Options, {Name, false}, Notify, Start) ->
    gen_server:Start(?MODULE, [Name, Notify, Options], []);
start_register(Options, {Name, Reg}, Notify, Start) ->
    gen_server:Start(Reg, ?MODULE, [Name, Notify, Options], []).

%% Node name and gen_server registration information
node_rego(Node, #{register := false}) ->
    {Node, false};
node_rego(undefined, _) ->
...
%% @private Reason return implementation
maybe_stop(Reason, #peer_state{options = #{report_reason := true}} = State) ->
...
handle_oob_data(_Kind, {started, NodeName}, #peer_state{options = Options} = State)->
    %% skip node name check if it was not specified
    case node_name(Options) of
        undefined ->
            boot_complete(NodeName, State);
        NodeName ->
            boot_complete(NodeName, State);
        Unexpected ->
...
boot_complete(_Node, #peer_state{notify = false} = State) ->
    State#peer_state{peer_state = running};
boot_complete(Node, #peer_state{notify = {ReplyTo, Tag}} = State) ->
    ReplyTo ! {Tag, {started, Node, self()}},
    State#peer_state{peer_state = running}.
...
command_line(Name, Listen, Options) ->
    ...
    %% build command line
    {ProgName, NameArg ++ StartCmd ++ CmdOpts}.
