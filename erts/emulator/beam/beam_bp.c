                                  ...
                                  int *return_to_trace)
{
    ...
    for (;;) {
        ...
        if (BeamIsReturnTrace(w)) {
            cpp += CP_SIZE + 2;
        } else if (BeamIsCallTraceReturn(w)) {
            cpp += CP_SIZE + 1;
...
void
erts_call_trace_return(Process *p, const ErtsCodeInfo *prev_info, int bp_flags)
{
    process_breakpoint_trace_t *pbt = NULL;
...
int erts_is_call_break(Process *p, int is_time, const ErtsCodeInfo *ci, Eterm *retval) {
    Uint i, ix;
	...
	if (retval) {
		...
		for(ix = 0; ix < hash.n; ix++) {
			...
			usec = is_time ? ERTS_MONOTONIC_TO_USEC(item->accumulator) : item->accumulator;
			sec = usec / 1000000;
