...
end_per_testcase(basic, _Config) ->
    erlang:trace_pattern({?MODULE, alloc_ref, 0}, false, [call_memory]),
...
all() ->
    ...
    spawn_memory_lambda, conflict_traces].
...
basic() ->
    [{doc, "Assert that erlang:make_ref() allocated 3-word structure on the heap"}].

basic(Config) when is_list(Config) ->
    Self = self(),
    Traced = {?MODULE, alloc_ref, 0},
    1 = erlang:trace_pattern(Traced, true, [call_memory]),
    1 = erlang:trace(Self, true, [call]),
    alloc_ref(),
    {call_memory, [{Self, 1, 0, 3}]} = erlang:trace_info(Traced, call_memory),
    alloc_ref(),
    {call_memory, [{Self, 2, 0, 6}]} = erlang:trace_info(Traced, call_memory),
    %% test that GC works correctly
    erlang:garbage_collect(),
    alloc_ref(),
    {call_memory, [{Self, 3, 0, 9}]} = erlang:trace_info(Traced, call_memory),
...
late_trace(Config) when is_list(Config) ->
    ...
    {call_memory, [{Pid, 1, 0, 12}]} = erlang:trace_info({?MODULE, late_trace_inner, 0}, call_memory),
    Pid ! stop,
    receive {'DOWN', MRef, process, Pid, _} -> ok end.

late_trace_inner() ->
    Ref = erlang:make_ref(), %% 3 words
...
skip(Config) when is_list(Config) ->
    ...
    1 = erlang:trace(Self, true, [call]),
    upper(),
    {call_memory, [{Self, 1, 0, 8 * 2}]} = erlang:trace_info({?MODULE, lower, 1}, call_memory),
    {call_memory, [{Self, 1, 0, 3 + 3 + 6}]} = erlang:trace_info({?MODULE, upper, 0}, call_memory).

upper() ->
    Ref = erlang:make_ref(),    %% 3
...
middle(_Ref) ->
    Ref2 = erlang:make_ref(),   %% 3 - but accounted in 'upper/0' instead
    lower(8),
    Ref2.
...
message(Config) when is_list(Config) ->
    ...
    receive done -> ok end,
    {call_memory, [{Pid, 1, 0, 9}]} = erlang:trace_info(Traced, call_memory),
    %% once again, just in case, to verify that decrementing "allocated" worked
    Pid ! pre,
    Pid ! NonLiteral,
    receive done -> ok end,
    {call_memory, [{Pid, 2, 0, 18}]} = erlang:trace_info(Traced, call_memory),
    1 = erlang:trace_pattern(Traced, false, [call_memory]).
...
parallel_map(Config) when is_list(Config) ->
    ...
    receive
        {'DOWN', MRef, process, Pid, _} ->
            %% alloc_ref called 3 times (once per process)
            {call_memory, [{_, 1, 0, 3}, {_, 1, 0, 3}, {_, 1, 0, 3}]} =
                erlang:trace_info({?MODULE, alloc_ref, 0}, call_memory),
...
do_parallel() ->
    Allocs = [spawn_link(fun() -> alloc_ref(), receive_message() end) || _ <- lists:seq(1, 3)],
...
profile_modules([{Module, _} | Tail], Acc) ->
    case profile_functions(Module, Module:module_info(functions), #{}) of
        Empty when Empty =:= #{} ->
...
collapse_procs(Processes) ->
    lists:foldl(
        fun ({_Pid, Calls, _Mega, Words}, {Procs, TotCalls, TotWords}) ->
...
spawn_memory(Config) when is_list(Config) ->
    ...
    {call_memory, [{Pid, 1, 0, 34}]} = erlang:trace_info({?MODULE, spawn_memory_internal, 1}, call_memory).
...
spawn_memory_lambda(Config) when is_list(Config) ->
    ...
    {call_memory, [{Pid, 1, 0, 40}]} = erlang:trace_info({erlang, apply, 2}, call_memory).
...
conflict_traces(Config) when is_list(Config) ->
    ...
    alloc_ref(), %% this goes to both time and memory
